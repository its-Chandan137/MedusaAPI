...
export const Hero = () => {
  ...
  return (
    <div>
      <section className="h-[90vh] lg:h-[90vh] mt-20 bg-white-100 relative z-1">
        <Slider {...settings}>
          {herolist.map((item) => (
            <HeroItem
              key={item.id}
...
export const HeroItem = ({ title, description, prices, colors, image }) => {
  ...
  return (
    <>
      <section className="content flex justify-between lg:px-16 h-fit lg:h-[90vh] relative z-20">
        <div className="left w-1/2 p-8 lg:p-32 lg:pt-20 lg:py-64">
          <Title
            level={1}
        ...
        <div className="right bg-white p-5 w-1/2 h-[90vh] flex justify-center items-center relative z-50">
          <img
            src={image}
            alt="image"
        ...
        <div
          className={`${
            className
              ...
              : "flex absolute bottom-12 p-8 w-full"
          } ${className && classSecond ? "left-0 lg:left-80 top-10 w-96" : ""}`}
        >
          ...
          <div className="w-1/2 mt-5">
            <NavLink to="/shop">
              <button  className="secondary-btn flex justify-end items-end w-36">shop now</button>
            </NavLink>
          </div>
        </div>
      </div>
